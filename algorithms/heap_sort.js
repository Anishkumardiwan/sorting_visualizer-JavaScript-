...
function swap(i, j) {
    ...
    div_update(divs[j], main_array[j], "blue");//Color update
}
...
function max_heapify(n, i) {
    ...
    if (l < n && main_array[l] > main_array[largest]) {
        if (largest != i) {
            div_update(divs[largest], main_array[largest], "blue");//Color update
        }

        largest = l;

        div_update(divs[largest], main_array[largest], "red");//Color update
    }
...
function heap_sort() {
    ...
    for (var i = array_size - 1; i > 0; i--) {
        swap(0, i);
        div_update(divs[i], main_array[i], "green");//Color update
        div_update(divs[i], main_array[i], "yellow");//Color update
